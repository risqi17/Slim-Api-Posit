...
use Tuupola\Base62;
	...
	$app->get("/detailtransaksi", function (Request $request, Response $response){
    	$sql = "SELECT pos_detail_transaction.*, pos_products.productname FROM pos_detail_transaction INNER JOIN pos_products ON pos_detail_transaction.productid=pos_products.productid";
    	$stmt = $this->db->prepare($sql);
    	$stmt->execute();
    	$result = $stmt->fetchAll();
    	return $response->withJson(["status" => "success", "data" => $result], 200);
